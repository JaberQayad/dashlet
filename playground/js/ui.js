...
export class UI {
    ...
    renderServices() {
        ...
        // Apply Sorting: by criteria only
        if (currentSettings.sortBy && currentSettings.sortBy !== 'manual') {
            allServices = [...allServices].sort((a, b) => {
                const valA = (a[currentSettings.sortBy] || '').toLowerCase();
                const valB = (b[currentSettings.sortBy] || '').toLowerCase();
        ...
        if (allServices.length === 0) {
            grid.innerHTML = '<div class="empty-state">No items added yet.</div>';
            return;
        }
        ...
        // Disable drag-drop if not in manual sort mode or if disabled in settings
        if (currentSettings.disableDragDrop || currentSettings.sortBy !== 'manual') return;
            ...
            input.onchange = (e) => {
                ...
                if (file) {
                    ...
                    reader.onload = (ev) => {
                        ...
                        let data = null;
                        try { data = JSON.parse(content); } catch (err) { alert('Failed to parse file: ' + err.message); return; }
                        if (data && data.settings) {
                            Object.keys(settings.settings).forEach(k => {
                                if (data.settings[k] !== undefined) settings.set(k, data.settings[k]);
                            });
                        }
                        if (data && data.services) services.replaceAll(data.services);
    ...
    applySettings(s) {
        ...
        if (s.accentColor) document.documentElement.style.setProperty('--primary-color', s.accentColor);
        if (s.wallpaper) {
        ...
        if (!s.blur) document.body.classList.add('no-blur'); else document.body.classList.remove('no-blur');
        if (!s.animations) document.body.classList.add('no-animations'); else document.body.classList.remove('no-animations');

        const grid = document.getElementById('grid');
        if (grid) {
            if (s.layout === 'list') grid.classList.add('list-view'); else grid.classList.remove('list-view');
        }
